...
namespace Tollbound.Model
{
    ...
    /// <summary>
    /// One line of a build recipe.
    ///
    /// A plain class rather than a value tuple on purpose: Valheim's Mono runtime ships no
    /// System.ValueTuple, so a tuple-typed field compiles cleanly against the reference
    /// assemblies and then throws TypeLoadException the first time the type is touched,
    /// taking the whole plugin down before anything registers.
    /// </summary>
    ...
    internal static class Tiers
    {
        internal static readonly TierInfo[] All =
        {
            new TierInfo
            {
                ...
                Recipe = new[] { new Ingredient("FineWood", 20), new Ingredient("GreydwarfEye", 10), new Ingredient("SurtlingCore", 2), new Ingredient("Copper", 5) },
